...
module ActiveFedora::File::Attributes
  ...
  def digest
    response = metadata.ldp_source.graph.query([nil, RDF::Vocab::PREMIS.hasMessageDigest, nil])
    # fallback on old predicate for checksum
    response = metadata.ldp_source.graph.query([nil, fallback_digest_predicate, nil]) if response.empty?
    response.map(&:object)
  end
