...
describe ActiveFedora::SimpleDatastream do
  ...
  describe "#to_solr" do
    it "should have title" do
      ...
      expect(solr[ActiveFedora::SolrService.solr_name('publisher', :string, :searchable)]).to eq(["publisher1"])
      expect(solr[ActiveFedora::SolrService.solr_name('creation_date', :date)]).to eq(["2012-01-15"])
    end
  end
