...
describe ActiveFedora::FileConfigurator do
  ...
  describe ".build_predicate_config_path" do
    it "should return the path to the default config/predicate_mappings.yml if no valid path is given" do
      expect(subject.send(:build_predicate_config_path, nil)).to eq(default_predicate_mapping_file)
    end

    it "should return the path to the default config/predicate_mappings.yml if specified config file not found" do
      expect(File).to receive(:exist?).with("/path/to/predicate_mappings.yml").and_return(false)
      expect(File).to receive(:exist?).with(default_predicate_mapping_file).and_return(true)
      expect(subject.send(:build_predicate_config_path,"/path/to")).to eq(default_predicate_mapping_file)
    end

    it "should return the path to the specified config_path if it exists" do
      expect(File).to receive(:exist?).with("/path/to/predicate_mappings.yml").and_return(true)
      expect(subject).to receive(:valid_predicate_mapping?).and_return(true)
      expect(subject.send(:build_predicate_config_path,"/path/to")).to eq("/path/to/predicate_mappings.yml")
    end
  end
  ...
  describe ".predicate_config" do
    ...
    it "should return the default mapping if it has not been initialized" do
      expect(subject.predicate_config()).to eq(YAML.load(File.read(default_predicate_mapping_file)))
    end
  end
