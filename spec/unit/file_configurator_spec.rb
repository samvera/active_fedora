  ...
  describe "setting the environment and loading configuration" do
    ...
    describe "within Rails" do
      ...
      it "loads a config from Rails.root as a first choice" do
        configurator.init
        expect(configurator.config_path(:fedora)).to eql(Rails.root.join("config/fedora.yml"))
        expect(configurator.solr_config_path).to eql(Rails.root.join("config/solr.yml"))
      end
