  ...
  describe "setting the environment and loading configuration" do
    ...
    describe "within Rails" do
      ...
      it "loads a config from Rails.root as a first choice" do
        configurator.init
        expect(configurator.config_path(:fedora)).to eql([Rails.root, "config/fedora.yml"].join('/'))
        expect(configurator.solr_config_path).to eql([Rails.root, "config/solr.yml"].join('/'))
      end
