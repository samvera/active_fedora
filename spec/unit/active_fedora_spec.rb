  ...
  describe ".init" do
    ...
    describe "outside of rails" do
      it "should load the default packaged config/fedora.yml file if no explicit config path is passed" do
        ActiveFedora.init()
        ActiveFedora.config[:url].should == "http://127.0.0.1:8983/fedora-test"
        ActiveFedora.config[:user].should == "fedoraAdmin"
        ActiveFedora.config[:password].should == "fedoraAdmin"
      end
      it "should load the passed config if explicit config passed in as a string" do
        ...
        ActiveFedora.config[:url].should == "http://testhost.com:8983/fedora"
        ActiveFedora.config[:user].should == "fedoraAdmin"
        ActiveFedora.config[:password].should == "fedoraAdmin"
      end
    end
