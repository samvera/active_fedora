...
describe ActiveFedora::MetadataDatastreamHelper do
  ...
  describe '#from_solr' do
    it 'should return an object with the appropriate metadata fields filled in' do
      ...
      expect(test_from_solr_object.descMetadata.language).to eq(["Italian"])
      expect(test_from_solr_object.descMetadata.creator).to eq(["Linguist, A."])
      expect(test_from_solr_object.descMetadata.geography).to eq(["Italy"])
      expect(test_from_solr_object.descMetadata.title).to eq(["Italian and Spanish: A Comparison of Common Phrases"])
      #test_from_solr_object.fields[:holding_id][:values].should == ["Holding 1"]

      expect(test_from_solr_object2.descMetadata.language).to eq(["Spanish;Latin"])
      expect(test_from_solr_object2.descMetadata.creator).to eq(["Linguist, A."])
      expect(test_from_solr_object2.descMetadata.geography).to eq(["Spain"])
      expect(test_from_solr_object2.descMetadata.title).to eq(["A study of the evolution of Spanish from Latin"])
      #test_from_solr_object2.fields[:holding_id][:values].should == ["Holding 2"]

      expect(test_from_solr_object3.descMetadata.language).to eq(["Spanish;Latin"])
      expect(test_from_solr_object3.descMetadata.creator).to eq(["Linguist, A."])
      expect(test_from_solr_object3.descMetadata.geography).to eq(["Spain"])
      expect(test_from_solr_object3.descMetadata.title).to eq(["An obscure look into early nomadic tribes of Spain"])
