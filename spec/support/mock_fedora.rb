...
def stub_add_ds(pid, dsids)
  pid.gsub!(/:/, '%3A')
  dsids.each do |dsid|
    allow(mock_client).to receive(:[]) { |params|
      /objects\/#{pid}\/datastreams\/#{dsid}/.match(params)
    }.and_return(double("ds_adder", :post=>pid, :get=>''))
  end
end
...
def stub_get_content(pid, dsids)
  ...
  dsids.each do |dsid|
    allow(mock_client).to receive(:[]) { |params|
      /objects\/#{pid}\/datastreams\/#{dsid}\/content/.match(params)
    }.and_return(double("content_accessor", :post=>pid, :get=>''))
  end
end
